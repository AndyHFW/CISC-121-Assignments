...
'''
All-encompassing top-level function
...
def main():
    ...
    while playersLeft>1:
        spinResult=spinWheel()                  #spin the wheel
        clearBets()                             #clear bets from the previous round
        makeBets()                              #make bets
        playersLeft=checkBets(playersLeft)      #check results
        showFunds()                             #show players' funds
        endGame()                               #outputs end game messages when 0 or 1 players left
